		 ...
		 unsigned int command, int column, int page)
{
	...
	if (command != NAND_CMD_READ0 &&
		...
		printf("Unhandled Command 0x%x, Page 0x%x, Column 0x%x\n",
			    command, page, column);
		return;
	}
		  ...
		  uint8_t *buffer, int page)
{
	...
	if (0 == bch_status) {
		debug("No errors in the BCH_STATUS register.\n");
		goto report_ecc_errors_end;
	}
...
board_nand_init(struct nand_chip *chip)
{
	...
	*/
	...
	if ((NULL == (a_to_i = malloc(sizeof(short) * 8192))) ||
		...
		printf("Couldn't allocate memory for ECC!\n");
		return -1;
	}
