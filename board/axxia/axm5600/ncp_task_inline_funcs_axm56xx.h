                ...
                void **taskAddr)
{
    ...
    debug("Enter mme alloc: request for %d bytes\r\n",size);
#endif
    ...
    debug("mme allocator (of matching pool) @ 0x%p, readP_swVA @ 0x%p\n",
...
mme_alloc_retry:
    ...
    *(pAllocIF->readP_swVA) = 0LL; /* Is this eally necessary ? */
    ...
    debug("new readP_swVA is 0x%p hwReadp=%llx\r\n",
        pAllocIF->readP_swVA,
        *(pAllocIF->readP_hwVA));
...
#ifdef oPCQ_DBG
	debug("pOutputQueueEntry %p\n", (void*)pOutputQueueEntry);
	debug("dealoc task @ taskAddr %p\n", (void*)taskAddr);
	debug(" ... port=%d, addr=%p, size=%d\n", 
                            ...
                            ncp_bool_t wait)                            
{
	...
	debug("what queue dealloc immediate is out on %p\n",(void*) pOutputQueueEntry);
 ...
 */    
    ...
    debug(" Dealloc Immediate: VP=%p", (void*)pDeallocImmediate->pduSegAddr0);   
#endif 
 ...
 */    
    ...
    while (1)
    {
        ...
        debug("Native NcaV2 Send: numPduSegs=%d: ", pTaskMetaData->ptrCnt);    
        switch(pTaskMetaData->ptrCnt)
        {
        case 6:
            NCP_MSG(NCP_MSG_INFO, 
