	...
	*/
	...
	while (0 < --max_loops) {
		...
		for (i = 1; i <= 4; ++i) {
			...
			ncr_read(NCP_REGION_ID(0x115, i), 0, 0x2022, 2, &temp);

			if (2 == ((temp & 0x180) >> 7)) {
				ncr_read(NCP_REGION_ID(0x115, i), 0, 0x202e,
					 2, &temp);
				...
				ncr_write(NCP_REGION_ID(0x115, i), 0, 0x200a,
					  2, &temp);
			}

			ncr_read(NCP_REGION_ID(0x115, i), 0, 0x2222, 2, &temp);

			if (2 == ((temp & 0x180) >> 7)) {
				ncr_read(NCP_REGION_ID(0x115, i), 0, 0x222e,
					 2, &temp);
				...
				ncr_write(NCP_REGION_ID(0x115, i), 0, 0x220a,
					  2, &temp);
			}
		}
