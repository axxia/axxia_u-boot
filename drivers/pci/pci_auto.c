	...
	pci_addr_t *bar)
{
	...
	DEBUGF("address=0x%llx bus_lower=0x%llx", (unsigned long long)addr,
			  ...
			  struct pci_region *io)
{
	...
	for (bar = PCI_BASE_ADDRESS_0;
		...
		if (bar_response & PCI_BASE_ADDRESS_SPACE) {
			...
			DEBUGF("PCI Autoconfig: BAR %d, %s, size=0x%llx, ",
	...
	if (header_type != PCI_HEADER_TYPE_CARDBUS) {
		...
		if (bar_response) {
			...
			DEBUGF("PCI Autoconfig: ROM, size=%#x, ",
...
int pciauto_config_device(struct pci_controller *hose, pci_dev_t dev)
{
	...
	case PCI_CLASS_BRIDGE_OTHER:
		 ...
		 */
		DEBUGF("PCI Autoconfig: Broken bridge found,
		       only minimal config\n");
